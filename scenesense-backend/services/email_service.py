...
def send_reset_otp_email(recipient_email: str, recipient_name: str, otp: str) -> None:
    """
    Send a password-reset OTP email to *recipient_email* via Brevo SMTP.
    ...
    subject = "SceneSense AI Password Reset OTP"

    plain_body = (
        f"Hi {recipient_name},\n\n"
        f"You requested a password reset for your SceneSense AI account.\n\n"
...
<head>
  ...
  <style>
    ...
    .header {{ background: linear-gradient(135deg, #06B6D4, #7C3AED); padding: 32px 40px; text-align: center; }}
    .header h1 {{ margin: 0; font-size: 22px; color: #fff; letter-spacing: -0.5px; }}
    .body {{ padding: 36px 40px; }}
    .greeting {{ font-size: 15px; color: #CBD5E1; margin-bottom: 24px; }}
    .otp-box {{ background: rgba(6,182,212,0.10); border: 1px solid rgba(6,182,212,0.4);
                border-radius: 12px; padding: 24px; text-align: center; margin: 24px 0; }}
    .otp-code {{ font-size: 42px; font-weight: 700; letter-spacing: 10px; color: #F1F5F9;
                 font-family: 'Courier New', monospace; }}
    .expiry {{ font-size: 13px; color: #94A3B8; margin-top: 10px; }}
    .warning {{ font-size: 12px; color: #F59E0B; margin-top: 18px;
...
<body>
  <div class="container">
    ...
    <div class="body">
      <p class="greeting">Hi <strong>{recipient_name}</strong>,<br/>
         You requested a password reset. Use the OTP below to set a new password.</p>
      <div class="otp-box">
        <div class="otp-code">{otp}</div>
        <div class="expiry">⏱ Expires in {config.OTP_EXPIRY_MINUTES} minutes</div>
      </div>
      <div class="warning">
        ⚠️ If you did not request this reset, please ignore this email.
        Your password will remain unchanged.
      </div>
      <p class="note">
        Enter this OTP on the password reset page.<br/>
...
"""
    ...
    try:
        with smtplib.SMTP(config.BREVO_SMTP_HOST, config.BREVO_SMTP_PORT, timeout=10) as server:
            ...
            server.login(config.BREVO_SMTP_LOGIN, config.BREVO_SMTP_PASSWORD)
            server.sendmail(config.BREVO_SENDER_EMAIL, recipient_email, msg.as_string())
        logger.info("Password reset OTP email sent to %s", recipient_email)
    except smtplib.SMTPException as exc:
        logger.error("Failed to send reset OTP email: %s", exc)
        raise RuntimeError(f"Email delivery failed: {exc}") from exc
    except OSError as exc:
        logger.error("SMTP connection error: %s", exc)
        raise RuntimeError(f"SMTP connection failed: {exc}") from exc
